...
version (NK_INCLUDE_FONT_BAKING) {
    ...
    * --------------------------------------------------------------*/
    ...
    enum nk_size nk_rect_align = stbrp_rect.alignof;
    enum nk_size nk_range_align = stbtt_pack_range.alignof;
    ...
    int nk_range_count(const(nk_rune)* range)
    {
        ...
        if (!range) return 0;
        while (*(iter++) != 0){}
        return (iter == range) ? 0 : cast(int)((iter - range)/2);
    }
    int nk_range_glyph_count(const(nk_rune)* range, int count)
    {
        int i = 0;
        int total_glyphs = 0;
        for (i = 0; i < count; ++i) {
    ...
    enum nk_rune[3] default_glyph_ranges = [0x0020, 0x00FF, 0];
    const(nk_rune)* nk_font_default_glyph_ranges()
    {
        return default_glyph_ranges.ptr;
    }

    enum nk_rune[11] chinese_glyph_ranges = [
        0x0020, 0x00FF,
        0x3000, 0x30FF,
        0x31F0, 0x31FF,
        0xFF00, 0xFFEF,
    ...
    {
        return chinese_glyph_ranges.ptr;
    }

    enum nk_rune[9] cyrillic_glyph_ranges = [
        0x0020, 0x00FF,
        0x0400, 0x052F,
        0x2DE0, 0x2DFF,
        0xA640, 0xA69F,
    ...
    {
        return cyrillic_glyph_ranges.ptr;
    }

    enum nk_rune[7] korean_glyph_ranges = [
        0x0020, 0x00FF,
        0x3131, 0x3163,
        0xAC00, 0xD79D,
        0
    ...
    {
        return korean_glyph_ranges.ptr;
    }
    ...
    void nk_font_baker_memory(nk_size* temp, int* glyph_count, nk_font_config* config_list, int count)
    {
        ...
        nk_font_config* iter = void, i = void;
        ...
        *temp = cast(nk_size)*glyph_count * stbrp_rect.sizeof;
        *temp += cast(nk_size)total_range_count * stbtt_pack_range.sizeof;
        *temp += cast(nk_size)*glyph_count * stbtt_packedchar.sizeof;
        *temp += cast(nk_size)count * nk_font_bake_data.sizeof;
        *temp += nk_font_baker.sizeof;
        *temp += nk_rect_align + nk_range_align + nk_char_align;
        *temp += nk_build_align + nk_baker_align;
    }
    
    pragma(mangle, "nk_font_baker")
    nk_font_baker* nk_font_baker_(void* memory, int glyph_count, int count, nk_allocator* alloc)
    {
        ...
        baker.packed_chars = cast(stbtt_packedchar*)nk_align_ptr((baker.build + count), nk_char_align);
        baker.rects = cast(stbrp_rect*)nk_align_ptr((baker.packed_chars + glyph_count), nk_rect_align);
        baker.ranges = cast(stbtt_pack_range*)nk_align_ptr((baker.rects + glyph_count), nk_range_align);
        baker.alloc = *alloc;
        return baker;
    }
    int nk_font_bake_pack(nk_font_baker* baker, nk_size* image_memory, int* width, int* height, nk_recti* custom, const(nk_font_config)* config_list, int count, nk_allocator* alloc)
    {
        ...
        for (config_iter = config_list; config_iter; config_iter = config_iter.next) {
            ...
            do {
                ...
                if (!stbtt_InitFont(font_info, cast(const(ubyte)*)it.ttf_blob, stbtt_GetFontOffsetForIndex(cast(const(ubyte)*)it.ttf_blob, 0)))
                    return nk_false;
            } while ((it = it.n) != config_iter);
        }
        *height = 0;
        *width = (total_glyph_count > 1000) ? 1024 : 512;
        stbtt_PackBegin(&baker.spc, null, cast(int)*width, cast(int)max_height, 0, 1, alloc);
        {
            ...
            if (custom) {
                ...
                stbrp_rect custom_space = void;
                nk_zero(&custom_space, custom_space.sizeof);
                custom_space.w = cast(stbrp_coord)(custom.w);
                ...
                *height = nk_max(*height, cast(int)(custom_space.y + custom_space.h));
                ...
                custom.w = cast(short)custom_space.w;
                custom.h = cast(short)custom_space.h;
            }
            ...
            for (input_i = 0, config_iter = config_list; input_i < count && config_iter;
                ...
                it = config_iter;
                do {int n = 0;
                    int glyph_count = void;
                    const(nk_rune)* in_range = void;
                    ...
                    tmp.range_count = cast(nk_rune)range_count;
                    range_n += range_count;
                    for (i = 0; i < range_count; ++i) {
                        in_range = &cfg.range[i * 2];
                        tmp.ranges[i].font_size = cfg.size;
                        tmp.ranges[i].first_unicode_codepoint_in_range = cast(int)in_range[0];
                        tmp.ranges[i].num_chars = cast(int)(in_range[1]- in_range[0]) + 1;
    ...
    void nk_font_bake(nk_font_baker* baker, void* image_memory, int width, int height, nk_font_glyph* glyphs, int glyphs_count, const(nk_font_config)* config_list, int font_count)
    {
        int input_i = 0;
        nk_rune glyph_n = 0;
        const(nk_font_config)* config_iter = void;
        const(nk_font_config)* it = void;
        ...
        nk_zero(image_memory, cast(nk_size)(cast(nk_size)width * cast(nk_size)height));
        baker.spc.pixels = cast(ubyte*)image_memory;
        baker.spc.height = cast(int)height;
        for (input_i = 0, config_iter = config_list; input_i < font_count && config_iter;
            config_iter = config_iter.next) {
            it = config_iter;
            do {const(nk_font_config)* cfg = it;
                nk_font_bake_data* tmp = &baker.build[input_i++];
                stbtt_PackSetOversampling(&baker.spc, cfg.oversample_h, cfg.oversample_v);
                stbtt_PackFontRangesRenderIntoRects(&baker.spc, &tmp.info, tmp.ranges, cast(int)tmp.range_count, tmp.rects);
            } while ((it = it.n) != config_iter);
        ...
        for (input_i = 0, config_iter = config_list; input_i < font_count && config_iter;
            ...
            do {nk_size i = 0;
                ...
                if (!cfg.merge_mode) {
                    ...
                    dst_font.ascent = (cast(float)unscaled_ascent * font_scale);
                    dst_font.descent = (cast(float)unscaled_descent * font_scale);
                ...
                for (i = 0; i < tmp.range_count; ++i) {
                    ...
                    for (char_idx = 0; char_idx < range.num_chars; char_idx++)
                    {
                        ...
                        stbtt_aligned_quad q = void;
                        nk_font_glyph* glyph = void;

                        /* query glyph bounds from stb_truetype */
                        const(stbtt_packedchar)* pc = &range.chardata_for_range[char_idx];
                        codepoint = cast(nk_rune)(range.first_unicode_codepoint_in_range + char_idx);
                        stbtt_GetPackedQuad(range.chardata_for_range, cast(int)width,
                            cast(int)height, char_idx, &dummy_x, &dummy_y, &q, 0);

                        /* fill own glyph type with data */
                        glyph = &glyphs[dst_font.glyph_offset + dst_font.glyph_count + cast(uint)glyph_count];
                        ...
                        if (cfg.coord_type == NK_COORD_PIXEL) {
                            glyph.u0 = q.s0 * cast(float)width;
                            glyph.v0 = q.t0 * cast(float)height;
                            glyph.u1 = q.s1 * cast(float)width;
                            glyph.v1 = q.t1 * cast(float)height;
                        } else {
                            glyph.u0 = q.s0;
                            glyph.v0 = q.t0;
                            glyph.u1 = q.s1;
                        ...
                        if (cfg.pixel_snap)
                            glyph.xadvance = cast(float)cast(int)(glyph.xadvance + 0.5f);
                        glyph_count++;
                    }
                }
    ...
    void nk_font_bake_custom_data(void* img_memory, int img_width, int img_height, nk_recti img_dst, const(char)* texture_data_mask, int tex_width, int tex_height, char white, char black)
    {
        nk_byte* pixels = void;
        int y = 0;
        int x = 0;
        ...
        pixels = cast(nk_byte*)img_memory;
    ...
    void nk_font_bake_convert(void* out_memory, int img_width, int img_height, const(void)* in_memory)
    {
        int n = 0;
        nk_rune* dst = void;
        const(nk_byte)* src = void;
        ...
        dst = cast(nk_rune*)out_memory;
        src = cast(const(nk_byte)*)in_memory;
        for (n = cast(int)(img_width * img_height); n > 0; n--)
            *dst++ = (cast(nk_rune)(*src++) << 24) | 0x00FFFFFF;
    }
    ...
    *
    * --------------------------------------------------------------*/
    float nk_font_text_width(nk_handle handle, float height, const(char)* text, int len)
    {
        ...
        nk_font* font = cast(nk_font*)handle.ptr;
        assert(font);
        assert(font.glyphs);
        if (!font || !text || !len)
            return 0;
        ...
        glyph_len = text_len = nk_utf_decode(text, &unicode, cast(int)len);
        if (!glyph_len) return 0;
        while (text_len <= cast(int)len && glyph_len) {
            const(nk_font_glyph)* g = void;
            ...
            /* offset next glyph */
            glyph_len = nk_utf_decode(text + text_len, &unicode, cast(int)len - text_len);
            text_len += glyph_len;
        }
        return text_width;
    }
    version (NK_INCLUDE_VERTEX_BUFFER_OUTPUT) {
    void nk_font_query_font_glyph(nk_handle handle, float height, nk_user_font_glyph* glyph, nk_rune codepoint, nk_rune next_codepoint)
    {
        float scale = void;
        const(nk_font_glyph)* g = void;
        nk_font* font = void;
    ...
    const(nk_font_glyph)* nk_font_find_glyph(nk_font* font, nk_rune unicode)
    {
        int i = 0;
        int count = void;
        int total_glyphs = 0;
        const(nk_font_glyph)* glyph = null;
        const(nk_font_config)* iter = null;
        ...
        do {count = nk_range_count(iter.range);
            for (i = 0; i < count; ++i) {
                ...
                int diff = cast(int)((t - f) + 1);
                if (unicode >= f && unicode <= t)
                    return &font.glyphs[(cast(nk_rune)total_glyphs + (unicode - f))];
                total_glyphs += diff;
            }
        } while ((iter = iter.n) != font.config);
        return glyph;
    }
    void nk_font_init(nk_font* font, float pixel_height, nk_rune fallback_codepoint, nk_font_glyph* glyphs, const(nk_baked_font)* baked_font, nk_handle atlas)
    {
        nk_baked_font baked = void;
        assert(font);
        assert(glyphs);
        assert(baked_font);
    ...
    *-----------------------------------------------------------------------------*/
    ...
    uint nk_decompress_length(ubyte* input)
    {
        return cast(uint)((input[8] << 24) + (input[9] << 16) + (input[10] << 8) + input[11]);
    }
    void nk__match(ubyte* data, uint length)
    {
    ...
    void nk__lit(ubyte* data, uint length)
    {
        assert (nk__dout + length <= nk__barrier);
        if (nk__dout + length > nk__barrier) { nk__dout += length; return; }
    ...
    uint nk_adler32(uint adler32, ubyte* buffer, uint buflen)
    {
        const(c_ulong) ADLER_MOD = 65521;
        c_ulong s1 = adler32 & 0xffff, s2 = adler32 >> 16;
        c_ulong blocklen = void, i = void;
    ...
    uint nk_decode_85_byte(char c)
    {
        return cast(uint)((c >= '\\') ? c-36 : c-35);
    }
    void nk_decode_85(ubyte* dst, const(ubyte)* src)
    {
        while (*src)
        {
            uint tmp = nk_decode_85_byte(cast(char)src[0]) +
                85 * (nk_decode_85_byte(cast(char)src[1]) +
                85 * (nk_decode_85_byte(cast(char)src[2]) +
                85 * (nk_decode_85_byte(cast(char)src[3]) +
                85 * nk_decode_85_byte(cast(char)src[4]))));

            /* we can't assume little-endianess. */
            dst[0] = cast(ubyte)((tmp >> 0) & 0xFF);
            dst[1] = cast(ubyte)((tmp >> 8) & 0xFF);
            dst[2] = cast(ubyte)((tmp >> 16) & 0xFF);
            dst[3] = cast(ubyte)((tmp >> 24) & 0xFF);
    ...
    * --------------------------------------------------------------*/
    ...
    {
        assert(atlas);
        assert(atlas.temporary.alloc && atlas.temporary.free);
        assert(atlas.permanent.alloc && atlas.permanent.free);
    ...
    version (NK_INCLUDE_STANDARD_IO) {
        nk_font* nk_font_atlas_add_from_file(nk_font_atlas* atlas, const(char)* file_path, float height, const(nk_font_config)* config)
        {
            nk_size size = void;
            char* memory = void;
            nk_font_config cfg = void;
    ...
    nk_font* nk_font_atlas_add_compressed(nk_font_atlas* atlas, void* compressed_data, nk_size compressed_size, float height, const(nk_font_config)* config)
    {
        uint decompressed_size = void;
        void* decompressed_data = void;
        nk_font_config cfg = void;
    ...
    nk_font* nk_font_atlas_add_compressed_base85(nk_font_atlas* atlas, const(char)* data_base85, float height, const(nk_font_config)* config)
    {
        int compressed_size = void;
    ...
    version (NK_INCLUDE_DEFAULT_FONT) {
        nk_font* nk_font_atlas_add_default(nk_font_atlas* atlas, float pixel_height, const(nk_font_config)* config)
        {
            assert(atlas);
            assert(atlas.temporary.alloc);
            assert(atlas.temporary.free);
    ...
    const(void)* nk_font_atlas_bake(nk_font_atlas* atlas, int* width, int* height, nk_font_atlas_format fmt)
    {
        int i = 0;
        void* tmp = null;
        nk_size tmp_size = void, img_size = void;
        nk_font* font_iter = void;
        nk_font_baker* baker = void;
        ...
        /* initialize each cursor */
        {
            ...
            for (i = 0; i < NK_CURSOR_COUNT; ++i) {
                ...
                cursor.img.w = cast(ushort)*width;
                cursor.img.h = cast(ushort)*height;
                cursor.img.region[0] = cast(ushort)(atlas.custom.x + nk_cursor_data[i][0].x);
                cursor.img.region[1] = cast(ushort)(atlas.custom.y + nk_cursor_data[i][0].y);
                cursor.img.region[2] = cast(ushort)nk_cursor_data[i][1].x;
                cursor.img.region[3] = cast(ushort)nk_cursor_data[i][1].y;
    ...
    void nk_font_atlas_end(nk_font_atlas* atlas, nk_handle texture, nk_draw_null_texture* tex_null)
    {
        ...
        if (tex_null) {
            tex_null.texture = texture;
            tex_null.uv.x = (atlas.custom.x + 0.5f)/cast(float)atlas.tex_width;
            tex_null.uv.y = (atlas.custom.y + 0.5f)/cast(float)atlas.tex_height;
        }
        for (font_iter = atlas.fonts; font_iter; font_iter = font_iter.next) {
    ...
    void nk_font_atlas_clear(nk_font_atlas* atlas)
    {
        ...
        if (atlas.config) {
            nk_font_config* iter = void, next = void;
            for (iter = atlas.config; iter; iter = next) {
                nk_font_config* i = void, n = void;
