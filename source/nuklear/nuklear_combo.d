...
nk_bool nk_combo_begin_text(nk_context* ctx, const(char)* selected, int len, nk_vec2 size)
{
    ...
    in_ = (win.layout.flags & NK_WINDOW_ROM || s == NK_WIDGET_ROM)? null: &ctx.input;
    if (nk_button_behavior_(&ctx.last_widget_state, header, in_, NK_BUTTON_DEFAULT))
        is_clicked = nk_true;
    ...
    default: break;}
    {
        ...
        int draw_button_symbol = void;

        nk_symbol_type sym = void;
    ...
    return nk_combo_begin(ctx, win, size, cast(nk_bool)is_clicked, header);
}
nk_bool nk_combo_begin_label(nk_context* ctx, const(char)* selected, nk_vec2 size)
{
    return nk_combo_begin_text(ctx, selected, nk_strlen(selected), size);
}
nk_bool nk_combo_begin_color(nk_context* ctx, nk_color color, nk_vec2 size)
{
    nk_window* win = void;
    nk_style* style = void;
    const(nk_input)* in_ = void;
...
nk_bool nk_combo_begin_symbol(nk_context* ctx, nk_symbol_type symbol, nk_vec2 size)
{
    ...
    nk_widget_layout_states s = void;
    const(nk_style_item)* background = void;
    nk_color sym_background = void;
    nk_color symbol_color = void;
...
nk_bool nk_combo_begin_symbol_text(nk_context* ctx, const(char)* selected, int len, nk_symbol_type symbol, nk_vec2 size)
{
    nk_window* win = void;
    nk_style* style = void;
    nk_input* in_ = void;
...
nk_bool nk_combo_begin_image(nk_context* ctx, nk_image img, nk_vec2 size)
{
    nk_window* win = void;
    nk_style* style = void;
    const(nk_input)* in_ = void;
...
nk_bool nk_combo_begin_image_text(nk_context* ctx, const(char)* selected, int len, nk_image img, nk_vec2 size)
{
    nk_window* win = void;
    nk_style* style = void;
    nk_input* in_ = void;
...
nk_bool nk_combo_begin_symbol_label(nk_context* ctx, const(char)* selected, nk_symbol_type type, nk_vec2 size)
{
    return nk_combo_begin_symbol_text(ctx, selected, nk_strlen(selected), type, size);
}
nk_bool nk_combo_begin_image_label(nk_context* ctx, const(char)* selected, nk_image img, nk_vec2 size)
{
    return nk_combo_begin_image_text(ctx, selected, nk_strlen(selected), img, size);
}
nk_bool nk_combo_item_text(nk_context* ctx, const(char)* text, int len, nk_flags align_)
{
    return nk_contextual_item_text(ctx, text, len, align_);
}
nk_bool nk_combo_item_label(nk_context* ctx, const(char)* label, nk_flags align_)
{
    return nk_contextual_item_label(ctx, label, align_);
}
nk_bool nk_combo_item_image_text(nk_context* ctx, nk_image img, const(char)* text, int len, nk_flags alignment)
{
    return nk_contextual_item_image_text(ctx, img, text, len, alignment);
}
nk_bool nk_combo_item_image_label(nk_context* ctx, nk_image img, const(char)* text, nk_flags alignment)
{
    return nk_contextual_item_image_label(ctx, img, text, alignment);
}
nk_bool nk_combo_item_symbol_text(nk_context* ctx, nk_symbol_type sym, const(char)* text, int len, nk_flags alignment)
{
    return nk_contextual_item_symbol_text(ctx, sym, text, len, alignment);
}
nk_bool nk_combo_item_symbol_label(nk_context* ctx, nk_symbol_type sym, const(char)* label, nk_flags alignment)
{
    return nk_contextual_item_symbol_label(ctx, sym, label, alignment);
}
void nk_combo_end(nk_context* ctx)
{
    nk_contextual_end(ctx);
}
void nk_combo_close(nk_context* ctx)
{
    nk_contextual_close(ctx);
}
int nk_combo(nk_context* ctx, const(char)** items, int count, int selected, int item_height, nk_vec2 size)
{
    int i = 0;
    int max_height = void;
    nk_vec2 item_spacing = void;
    nk_vec2 window_padding = void;
    ...
    max_height = count * item_height + count * cast(int)item_spacing.y;
    max_height += cast(int)item_spacing.y * 2 + cast(int)window_padding.y * 2;
    size.y = nk_min(size.y, cast(float)max_height);
    if (nk_combo_begin_label(ctx, items[selected], size)) {
        nk_layout_row_dynamic(ctx, cast(float)item_height, 1);
        for (i = 0; i < count; ++i) {
...
int nk_combo_separator(nk_context* ctx, const(char)* items_separated_by_separator, int separator, int selected, int count, int item_height, nk_vec2 size)
{
    int i = void;
    int max_height = void;
    nk_vec2 item_spacing = void;
    nk_vec2 window_padding = void;
    const(char)* current_item = void;
    const(char)* iter = void;
    int length = 0;
    ...
    for (i = 0; i < count; ++i) {
        ...
        length = cast(int)(iter - current_item);
        if (i == selected) break;
        current_item = iter + 1;
    }
...
int nk_combo_string(nk_context* ctx, const(char)* items_separated_by_zeros, int selected, int count, int item_height, nk_vec2 size)
{
    return nk_combo_separator(ctx, items_separated_by_zeros, '\0', selected, count, item_height, size);
}
int nk_combo_callback(nk_context* ctx, void function(void*, int, const(char)**) @nogc nothrow item_getter, void* userdata, int selected, int count, int item_height, nk_vec2 size)
{
    int i = void;
    int max_height = void;
    nk_vec2 item_spacing = void;
...
void nk_combobox(nk_context* ctx, const(char)** items, int count, int* selected, int item_height, nk_vec2 size)
{
    *selected = nk_combo(ctx, items, count, *selected, item_height, size);
}
void nk_combobox_string(nk_context* ctx, const(char)* items_separated_by_zeros, int* selected, int count, int item_height, nk_vec2 size)
{
    *selected = nk_combo_string(ctx, items_separated_by_zeros, *selected, count, item_height, size);
}
void nk_combobox_separator(nk_context* ctx, const(char)* items_separated_by_separator, int separator, int* selected, int count, int item_height, nk_vec2 size)
{
    *selected = nk_combo_separator(ctx, items_separated_by_separator, separator,
                                    *selected, count, item_height, size);
}
void nk_combobox_callback(nk_context* ctx, void function(void* data, int id, const(char)** out_text) @nogc nothrow item_getter, void* userdata, int* selected, int count, int item_height, nk_vec2 size)
{
